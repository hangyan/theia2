...
start_antrea() {
  ...
  mkdir -p ${GIT_CHECKOUT_DIR}/build/yamls/
  test -f ${GIT_CHECKOUT_DIR}/build/yamls/antrea.yml || wget -c https://raw.githubusercontent.com/antrea-io/antrea/main/build/yamls/antrea.yml -O ${GIT_CHECKOUT_DIR}/build/yamls/antrea.yml
...
check_grafana() {
  ...
  if ( echo ${grafana_logs} | grep -q 'msg="Plugin registered" logger=plugin.manager pluginId=theia-grafana-sankey-plugin' ); then
...
config_antrea() {
  ...
  kubectl delete -f  https://raw.githubusercontent.com/antrea-io/antrea/main/build/yamls/antrea.yml
...
check_record() {
  ...
  do
    ...
    clickhouse_output=$(kubectl exec -i chi-clickhouse-clickhouse-0-0-0  -n flow-visibility -- clickhouse-client -h clickhouse-clickhouse.flow-visibility.svc -u clickhouse_operator --password clickhouse_operator_password --query="select count(*) from flows")
